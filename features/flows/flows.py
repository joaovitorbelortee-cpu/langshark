...
Fontes:
  - Supabase (tabela public.flows) — produção, gerenciado por painel admin
  - In-memory registry (FLOW_REGISTRY) — dev/teste, fallback se Supabase offline

Cache 60s nos fetches Supabase.
...
Disparo:
  LLM emite tag [FLOW: nome] no final da resposta.
  parse_flow_tag() extrai → flow_executor envia sequência.
"""
...
def register_flow(project_id: str, flow: Flow) -> None:
    """Registra flow em memória — útil pra testes ou setups sem Supabase."""
    FLOW_REGISTRY.setdefault(project_id, {})[flow.name.lower()] = flow


# ──────────────────────────────────────────────────────────────────────
# Supabase fetch + cache (60s TTL, LRU 100 projects)
# ──────────────────────────────────────────────────────────────────────

_FLOWS_CACHE: dict[str, tuple[list[Flow], float]] = {}
_FLOWS_CACHE_TTL = 60.0
_FLOWS_CACHE_MAX = 100
...
def _fetch_flows_supabase(project_id: str) -> list[Flow]:
    """Síncrono. Retorna [] se Supabase indisponível."""
    ...
    try:
        ...
        log.warning("[flows] supabase fetch falhou (%s) — fallback registry", exc)
        return []
...
def _cached_flows(project_id: str) -> list[Flow]:
    """Cache 60s. Combina Supabase + REGISTRY (Supabase wins em conflito)."""
    now = time.time()
    ...
    if merged:
        log.info("[flows] %s: %d flow(s) — %s", project_id, len(merged),
                 [f.name for f in merged])
    return merged


def invalidate_flows_cache(project_id: str | None = None) -> None:
    """Limpa cache (chamado por endpoint admin pós-mutation)."""
    if project_id:
...
def get_flow(project_id: str, name: str) -> Flow | None:
    """Match case-insensitive em Supabase + registry (cache 60s)."""
    if not name:
        return None
...
def parse_flow_tag(text: str) -> tuple[str | None, str]:
    """Extrai tag [FLOW: nome] e devolve (flow_name, texto_limpo)."""
...
def flows_prompt_block(project_id: str) -> str:
    """Bloco descritivo dos fluxos pra injetar no system prompt do LLM."""
